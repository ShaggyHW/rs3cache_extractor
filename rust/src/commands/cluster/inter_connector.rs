...
pub fn build_inter_edges(tiles_db: &Connection, out_db: &mut Connection, cfg: &Config) -> Result<InterStats> {
    ...
    let mut entrances: Vec<(i64, i32, i32, i32, char)> = Vec::new();
    {
        let mut q = out_db.prepare(
            "SELECT entrance_id, x, y, plane, neighbor_dir FROM cluster_entrances\n             WHERE neighbor_dir IN ('N','S','E','W')\n             ORDER BY plane, x, y, entrance_id",
        )?;
...
mod tests {
    ...
    fn creates_bidirectional_edges_with_cardinal_cost() -> Result<()> {
        ...
        tiles.execute(
            r#"INSERT INTO tiles(x,y,plane,flag,blocked,walk_mask,blocked_mask,walk_data)
                VALUES (63,0,0,0,0,1,0,'{"top":true,"bottom":true,"left":true,"right":true}')"#,
            [],
        )?;
        tiles.execute(
            r#"INSERT INTO tiles(x,y,plane,flag,blocked,walk_mask,blocked_mask,walk_data)
                VALUES (64,0,0,0,0,1,0,'{"top":true,"bottom":true,"left":true,"right":true}')"#,
            [],
        )?;

        // Entrances on each side of border
        out.execute("INSERT INTO clusters(cluster_id, plane, label, tile_count) VALUES (1,0,0,1)", [])?;
        out.execute("INSERT INTO clusters(cluster_id, plane, label, tile_count) VALUES (2,0,0,1)", [])?;
