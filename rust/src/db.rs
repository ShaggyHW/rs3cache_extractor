...
pub fn create_tables(conn: &mut Connection) -> Result<()> {
    ...
    if schema_has_user_tables(conn)? {
        bail!("Database already contains tables; delete the existing file before running.");
    }
...
fn schema_has_user_tables(conn: &Connection) -> Result<bool> {
