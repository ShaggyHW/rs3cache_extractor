...
import { exportWalkFlags } from "./scripts/exportwalk";
...
export function cliApi(ctx: CliApiContext) {
	...
	const walkflags = command({
		name: "walkflags",
		args: {
			...filesource,
			...saveArg("walk"),
			startx: option({ long: "startx", type: cmdts.number, defaultValue: () => 0 }),
			startz: option({ long: "startz", type: cmdts.number, defaultValue: () => 0 }),
			sizex: option({ long: "sizex", type: cmdts.number, defaultValue: () => 128 }),
			sizez: option({ long: "sizez", type: cmdts.number, defaultValue: () => 200 })
		},
		async handler(args) {
			let output = ctx.getConsole();
			let source = await args.source();
			await output.run(exportWalkFlags, args.save, source, args.startx, args.startz, args.sizex, args.sizez);
		}
	});
	...
	let subcommands = cmdts.subcommands({
		name: "",
		cmds: { extract, indexoverview, testdecode, diff, quickchat, scrapeavatars, edit, historicdecode, openrs2ids, filehist, cluecoords, sequencegroups, gameinterfaces, clientscriptmodule, walkflags }
	});
